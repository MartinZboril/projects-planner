        ...
        <ul class="todo-list" data-widget="todo-list" id="{{ $type }}-items-list">
            @foreach ($items as $item)
                <li id="{{ $type }}-item-{{ $item->get('type') }}-{{ $item['id'] }}">
                    ...
                    <small class="badge badge-{{ $item['item']['deadline_overdue'] ? 'danger' : 'secondary' }}" id="{{ $type }}-{{ $item['id'] }}-due-date"><i class="far fa-clock mr-1"></i>{{ $item['dued_at']->format('d.m.Y') }}</small>
                    <div class="tools">
