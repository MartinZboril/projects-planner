<ul class="todo-list ui-sortable" data-widget="todo-list">
    @foreach ($todos as $todo)
        <li>
            ...
            @endif
            <div class="tools">
                <a href="{{ route('projects.tasks.todos.edit', ['project' => $todo->task->project, 'task' => $todo->task, 'todo' => $todo->id]) }}"><i class="fas fa-edit"></i></a>
            </div>
            <!-- ToDos forms -->
            @include('projects.tasks.todos.forms.check', ['id' => 'check-todo-' . $todo->id . '-form', 'todo' => $todo, 'action' => $todo->is_finished ? 0 : 1])            
        </li>
    @endforeach
</ul>