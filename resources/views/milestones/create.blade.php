                            ...
                            <div class="card card-primary card-outline">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group required">
                                        ...
                                        <select class="form-control @error('owner_id') is-invalid @enderror" name="owner_id" id="owner-id" style="width: 100%;">
                                            ...
                                            @foreach($project->team as $user)
                                                <option value="{{ $user->id }}" @selected(old('owner_id') == $user->id)>{{ $user->full_name }}</option>
                                            @endforeach
                                        </select>
                                        @error('owner_id')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                    ...
                                    <div class="form-group required">
                                        ...
                                        <input type="date" name="end_date" id="end_date" class="form-control @error('end_date') is-invalid @enderror" placeholder="due date" value="{{ old('end_date', date('Y-m-d', strtotime('+7 days'))) }}" autocomplete="off">
                                        @error('end_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
